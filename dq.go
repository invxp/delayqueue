...
type DelayQueue struct {
	...
	saveTaskHandler func(taskID, taskType string, timestamp, cycle, pos int64) //保存任务回调
...
func New(options ...HandlerOptions) *DelayQueue {
	dq := &DelayQueue{wheelSize: defaultWheelSize, fun: hashmap.New(hashmap.DefaultSize), close: make(chan struct{})}
	...
	for i := int64(0); i < dq.wheelSize; i++ {
...
func (dq *DelayQueue) Push(delaySeconds int64, taskType string) {
	...
	f := dq.searchFunction(taskType)
	now := time.Now().Unix()
	if dq.ensureValidTask(delaySeconds, now, taskID, taskType, f) {
		dq.saveTask(dq.push(delaySeconds, now, taskID, taskType, f, true))
	}
}
...
func (dq *DelayQueue) AfterFunc(delaySeconds int64, f func()) {
	taskID := strings.ReplaceAll(uuid.NewString(), "-", "")
	now := time.Now().Unix()
	if dq.ensureValidTask(delaySeconds, now, taskID, taskID, f) {
		dq.push(delaySeconds, now, taskID, taskID, f, false)
	}
}
